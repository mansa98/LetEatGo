...
function isCheckFldValid(fld, regex, initVal, errMsg) {
    ...
    // 폼 유효성 점검(test)
    console.log(`유효성 점검 여부 : ${regex.test(fld.value)}`);
...
window.onload = function() {
	...
	for (role_update_btn of role_update_btns) {
		...
		role_update_btn.onclick = function(e) {
			...
			axios.get(`/member/updateRoles/${id}/roleUser/${roleUser.checked}/roleAdmin/${roleAdmin.checked}`)
	...
	for (enabledUpdateBtn of enabledUpdateBtns) {
		...
		enabledUpdateBtn.onclick = (e) => {
			...
			axios.get(`/member/changeMemberState/${id}/${enabled}`)
				 .then(function(response) {
	...
	// 연락처(휴대폰) 수정 관련
	// 'mobile_'
	let mobileUpdateFlds = document.querySelectorAll("[id^=mobile_]");
	
	for (mobileUpdateFld of mobileUpdateFlds) {
		...
		mobileUpdateFld.onclick = (e) => {
			...
			let mobileFld = document.getElementById('mobile_'+id);
			
			// 기존 의 모바일 값 보관
			defaultMobile = mobileFld.value;
			
			// 휴대폰 표시 영역(td)를 편집 가능한 상태로 변경
			mobileFld.removeAttribute("readonly");			
			mobileFld.focus();
		}
		
		// 벗어났을 때 => 편집 불가능 상태(readonly)
		mobileUpdateFld.onblur = (e) => {
			...
			// 수정 작성시 미완성후 blur 했을 경우 원래 값으로 초기화
			let regexMobile = /^010-\d{4}-\d{4}$/
			
			if (regexMobile.test(mobileFld.value) == false){
				
				mobileFld.value = defaultMobile;
				
			}
			
			
			// 휴대폰 표시 영역(td)를 편집 불가능한 상태(초기 상태)로 변경
			mobileFld.setAttribute("readonly", "readonly");
		}	
	...
	for (updateBtn of updateBtns) {
		...
		updateBtn.onclick = (e) => {
			...
			console.log("휴대폰 : ", mobileFld.value);
			...
			// 개별 모바일 체크 플래그
			let mobileDuplicatedCheckFlag = document.getElementById("check_mobile_" + id);
			
			// 유효성 점검을 통과해야만 전송
			if (mobileCheckFlag == true) { // 유효한 휴대폰 정보라면...
				
				/////////////////////////////////////////////////////////////////////////////////////////////////////////
				
				axios.get(`/member/hasFldForUpdateByAdmin/${id}/mobile/${mobileFld.value}`)
					 .then(function(response) {
					
						// 오류 있을 때만 팝업으로 출력하도록 조치
						if (response.data == true) {
							
							alert("중복되는 연락처(휴대폰)가 존재합니다. 다른번호를 입력하세요.");
							
							// 기존 번호로 초기화
							mobileFld.value = defaultMobile;
							
						} else { // 오류가 없을 때 (중복이 안될 때)
							
							alert("중복되지 않은 사용 가능한 번호 입니다.");
							
							console.log("휴대폰 정보 수정 전송 (mobileFld) : " + mobileFld.value);
							console.log("휴대폰 정보 수정 전송 (id) : " + id);
							
							axios({
								method : "post",
								url : '/member/updateMemberByAdmin',
								data : {
									id : id,	
							 ...
							 .then(function(response) {
								...
								msg = (response.data === true) ? "회원 정보 수정 처리에 성공하였습니다." : "회원 정보 수정 처리에 실패하였습니다.";
	...
	for (deleteBtn of deleteBtns) {
		...
		deleteBtn.onclick = (e) => {
			...
			if (confirm(`${id} 님의 회원 정보를 정말 삭제하시겠습니까?`) == true) {
				...
				axios.get(`/member/deleteMemberByAdmin/${id}`)
				 .then(function(response) {
	...
	let deleteMyIdBtns = document.querySelectorAll("[id^=delete_my_btn]");
	
	for (deleteMyIdBtn of deleteMyIdBtns) {
		
		deleteMyIdBtn.onclick = (e) => {
