...
public class MemberUpdateSessController {
	...
	public String update(Model model, HttpSession session ) {
		...
		String id = customUser.getUsername(); // 가져온 member_id를 id로 바꿈
		
		MemberDTO memberDTO;
		
		// session.removeAttribute("memberUpdateDTO");
		
		log.info("session memverUpdateDTO : {}", session.getAttribute("memberUpdateDTO"));
	...
	public String updateProc(@ModelAttribute("memberUpdateDTO") MemberUpdateDTO memberUpdateDTO, 
		...
		if (memberUpdateDTO.getPw1().trim().equals("") != true) {
			...
			memberUpdateDTO.setPw2(bCryptPasswordEncoder.encode(memberUpdateDTO.getPw1()));				
		}
